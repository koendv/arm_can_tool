 ...
 *Iand using the menu set:
 * Target->file i/o->On
 * Target->shell command->On
 */
...
void check_heapinfo() {
  ...
  heapinfo_block_t heapinfo = { (int32_t)0xdeadbeef };
  sys_heapinfo(&heapinfo);
  sh.print("heap_base: ");
  sh.println(heapinfo.heap_base, HEX);
  sh.print("heap_limit: ");
  sh.println(heapinfo.heap_limit, HEX);
  sh.print("stack_base: ");
  sh.println(heapinfo.stack_base, HEX);
  sh.print("stack_limit: ");
  sh.println(heapinfo.stack_limit, HEX);
}

void check_system() {
  // (gdb) set remote system-call-allowed 1
  sh.println("SYS_SYSTEM");
  char cmd[] = "echo \"system call works\"";
  int ret = sys_system(cmd, strlen(cmd));
  if (ret != 0) sh.println("Set Target->shell command->on");
  check_retcode(stdin_handle);
}
