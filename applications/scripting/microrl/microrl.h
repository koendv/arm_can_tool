...
#if MICRORL_CFG_USE_RAW_CALLBACKS

/**
 * \brief           Command execute callback function
 * Raw callback mode - receives raw command line buffer
 * \param[in]       mrl: microRL working instance
 * \param[in]       buf: Raw command line buffer
 * \param[in]       buflen: Length of command line buffer
 * \return          '0' on success, non-zero otherwise
 */
typedef int       (*microrl_exec_fn)(struct microrl* mrl, const char* buf, uint32_t buflen);

/**
 * \brief           Auto-complete function prototype
 * Raw callback mode - receives raw buffer, returns match count
 * \param[in,out]   mrl: \ref microrl_t working instance
 * \param[in]       buf: Current command line buffer
 * \param[in]       buflen: Length of current command line
 * \return          '0' on success, non-zero otherwise
 */
typedef int       (*microrl_get_compl_fn)(struct microrl* mrl, char* buf, uint32_t buflen);
...
microrlr_t  microrl_redraw_terminal(microrl_t* mrl);
