...
/* execution callback with raw buffer */
static microrlr_t prv_handle_newline(microrl_t* mrl) {
    ...
    /* Call execution callback with raw buffer */
    exec_status = mrl->exec_fn(mrl, mrl->cmdline_str, mrl->cmdlen);
...
/* execution callback with argv, argc */
static microrlr_t prv_handle_newline(microrl_t* mrl) {
...
static microrlr_t prv_complete_get_input(microrl_t* mrl) {
    ...
    /* Only complete when cursor is at the end of line */
    if (mrl->cursor != mrl->cmdlen) {
        return microrlOK;
    }
    ...
    int comp_status = mrl->get_completion_fn(mrl, mrl->cmdline_str, mrl->cmdlen);
...
#endif /* MICRORL_CFG_USE_RAW_CALLBACKS */
...
/**
 * \brief           Redraw the current prompt and command line
 * \param[in,out]   mrl: \ref microrl_t working instance
 * \return          \ref microrlOK on success, member of \ref microrlr_t enumeration otherwise
 */
