...
void ds3231_sync()
{
    ...
    if ((year <= 0) || (mon == 0) || (mon > 12) || (mday == 0) || (mday > 31) || (hour > 23) || (min > 59) || (sec > 60))
    {
...
void ds3231_date(int argc, char **argv)
{
    ...
    if (argc < 7)
    {
        ...
        { // ds3231 clock invalid?
            ...
            rt_kprintf("Use: ds3231 date year mon day hour min sec\r\n", argv[0]);
            return;
        }
...
static int ds3231(int argc, char **argv)
{
    if ((argc == 2 || argc == 8) && !strncmp(argv[1], "date", strlen(argv[1])))
    {
        ds3231_date(--argc, ++argv);
    }
    else if (argc == 2 && !strncmp(argv[1], "sync", strlen(argv[1])))
        ds3231_sync();
    else if (argc == 2 && !strncmp(argv[1], "temp", strlen(argv[1])))
        ds3231_temp();
    else
    {
        rt_kprintf("ds3231 (date|sync|temp)\r\n");
        rt_kprintf("ds3231 date : print ds3231 time\r\n");
        rt_kprintf("ds3231 date year mon day hour min sec: set ds3231 time\r\n");
        rt_kprintf("ds3231 sync : set rtc time to ds3231 time\r\n");
        rt_kprintf("ds3231 temp : print ds3231 temperature\r\n");
    }
    return RT_EOK;
}

MSH_CMD_EXPORT(ds3231, set real - time clock);

/* for mui interface */

void clock_get(uint8_t *year, uint8_t *mon, uint8_t *mday, uint8_t *hour, uint8_t *min, uint8_t *sec)
{
    rt_err_t   ret = RT_EOK;
    rt_uint8_t buff[7];
    rt_uint8_t status_reg;
    ...
    *sec  = 0;
    ...
    ret = ds3231_read_reg(DS3231_REG_STATUS, &status_reg, 1);
    if (ret != RT_EOK) return;
    if (status_reg >> 7) return; // ds3231 clock invalid?
...
void clock_set(uint8_t year, uint8_t mon, uint8_t mday, uint8_t hour, uint8_t min, uint8_t sec)
{
    rt_err_t   ret = RT_EOK;
    rt_uint8_t wday;
    rt_uint8_t buff[7];
