...
const uint32_t polling_interval_ms[] = {1, 2, 5, 10, 20, 50, 100, 200, 500};      /* target polling interval in milliseconds */
...
uint8_t mui_target_polling_interval(mui_t *ui, uint8_t msg)
{
    uint8_t retval = mui_u8g2_u8_opt_line_wa_mud_pi(ui, msg);
    if (settings.polling_interval >= sizeof(polling_interval_ms) / sizeof(polling_interval_ms[0]))
        settings.polling_interval = DEFAULT_POLLING;
    if ((msg == MUIF_MSG_EVENT_NEXT) || (msg == MUIF_MSG_EVENT_PREV))
        gdb_set_polling_interval_ms(polling_interval_ms[settings.polling_interval]);
    return retval;
}
...
uint8_t mui_can1_set_speed(mui_t *ui, uint8_t msg)
{
    ...
    if ((msg == MUIF_MSG_EVENT_NEXT) || (msg == MUIF_MSG_EVENT_PREV))
        ...
        {
            settings.can1_speed = 0;
            can_set_bitrate_index(settings.can1_speed);
        }
    return retval;
}
...
uint8_t mui_memory_info(mui_t *ui, uint8_t msg)
{
    ...
    if (msg == MUIF_MSG_DRAW)
    {
        ...
        snprintf(buf, sizeof(buf), "%d%s", mem_free, s);
        buf[sizeof(buf) - 1] = '\0';
...
muif_t muif_list[] = {
    ...
    /* enable auto start */
    MUIF_CHECKBOX_LIST("CE", startup_items_en, STARTUP_ITEMS_COUNT,
                       mui_checkbox_list_w1_pi),
    MUIF_CHECKBOX_LIST("CZ", startup_items_zh, STARTUP_ITEMS_COUNT,
                       mui_checkbox_list_w1_pi),
    /* enable target power */
    ...
    /* debugger polling interval - polling checks whether target still running */
    MUIF_VARIABLE("T2", &settings.polling_interval, mui_target_polling_interval),
    /* allow semihosting to read and write files */
    MUIF_VARIABLE("T3", &settings.fileio_enable, mui_u8g2_u8_chkbox_wm_pi),
    /* allow semihosting to execute shell commands */
    MUIF_VARIABLE("T4", &settings.shell_enable, mui_u8g2_u8_chkbox_wm_pi),
    /* target serial speed */
    ...
    /* canbus logging output enable */
    MUIF_VARIABLE("C1", &settings.can1_log, mui_u8g2_u8_chkbox_wm_pi),
    /* canbus filtering enable */
    MUIF_VARIABLE("C2", &settings.canfilter_enable, mui_u8g2_u8_chkbox_wm_pi),
    /* from usb cdc0 to target */
    MUIF_VARIABLE("U1", &settings.cdc0_out, mui_u8g2_u8_opt_line_wa_mud_pi),
    ...
    /* settings preset: 0..15 */
    MUIF_U8G2_U8_MIN_MAX("XS", &settings_preset, 0, SETTINGS_MAX_PRESET, mui_u8g2_u8_min_max_wm_mud_pi),
    /* print free ram */
...
static void mui_setup()
{
    ...
    u8g2_FirstPage(&u8g2);
    do {
        u8g2_DrawStr(&u8g2, 0, 15, "ARM CAN TOOL");
        u8g2_DrawUTF8(&u8g2, 0, 31, "ready 就绪");
    } while (u8g2_NextPage(&u8g2));

    /* choose fds_data language according to settings.language */

    static fds_t * const fds_languages[] = {
        fds_data_en, /* 0:  en  */
        fds_data_zh, /* 1: 中文 */
    };

    fds_t *fds = (settings.language < sizeof(fds_languages) / sizeof(fds_languages[0]))
                     ? fds_languages[settings.language]
                     : fds_data_en; /* fallback to english if setting is out of range */

    mui_Init(&mui, &u8g2, fds, muif_list, sizeof(muif_list) / sizeof(muif_t));
